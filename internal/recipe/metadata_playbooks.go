...
func conflictPlaybook() PlaybookDefinition {
	return PlaybookDefinition{
		...
		Steps: []PlaybookStep{
			...
			{ID: "release_to_human", Kind: "manifest_edit", Effect: "repository_mutation", Summary: "To keep human content, add the path to ownership.release in bob.yaml so Bob seeds it once and never lock-owns it again", Paths: []string{"bob.yaml"}, Argv: []string{}, DependsOn: []string{"choose_intent"}, RequiresExplicitAuthority: true, SuccessCondition: "bob.yaml lists the path under ownership.release and validates", BlockedBy: []string{}},
			{ID: "review_plan", Kind: "bob_plan", Effect: "read_only", Summary: "Review the plan after releasing the path", Paths: []string{"<path>"}, Argv: []string{"bob", "plan", "<workspace>", "--json"}, DependsOn: []string{"release_to_human"}, RequiresExplicitAuthority: false, SuccessCondition: "the released path is seed_exists or absent from conflicts", BlockedBy: []string{}},
			{ID: "apply_release", Kind: "bob_apply", Effect: "repository_mutation", Summary: "Apply the reviewed plan so bob.lock drops the released path", Paths: []string{"bob.yaml", "bob.lock"}, Argv: []string{"bob", "apply", "<workspace>", "--expect-plan-digest", "<reviewed_plan_digest>", "--json"}, DependsOn: []string{"review_plan"}, RequiresExplicitAuthority: true, SuccessCondition: "Bob applies the reviewed digest and no longer lock-owns the released path", BlockedBy: []string{}},
		},
